class CountyDataController
    ...
    def self.select_county_input(state_input)
        ...
        if (1..CountyData.list_all_county_names_in_one_state(state_input).length).include?(county_input)
            county = CountyData.list_all_county_names_in_one_state[county_input - 1]
            county_data = CountyData.list_county_data(county, state_input)
        end
    end
end